...
impl ProgressEvent {
    ...
    pub fn merging(pct: Option<f32>) -> Self {
        Self::Merging { pct }
    }
    pub fn done(output: PathBuf, size_mb: f64) -> Self {
    ...
    pub fn to_job_state(&self) -> JobState {
        match self {
            ...
            Self::Done { output, size_mb } => JobState::Done {
                output: OutputPath(output.clone()),
                size_mb: *size_mb,
            },
            Self::Failed { error } => JobState::Failed {
                error: error.clone(),
            },
        }
    }
