...
impl CollectingSink {
    ...
    fn snapshot(&self) -> Vec<ProgressEvent> { self.0.lock().expect("lock").clone() }
}
impl ProgressSink for CollectingSink {
    #[allow(clippy::expect_used)]
    fn emit(&self, e: ProgressEvent) { self.0.lock().expect("lock").push(e); }
}

#[allow(clippy::expect_used)] // tests/* helper
struct ConcatMuxer;
impl Muxer for ConcatMuxer {
    async fn mux(&self, inputs: &OrderedSegments, output: &Path, _dur: f64, sink: &dyn ProgressSink) -> Result<()> {
        let mut out = Vec::new();
...
async fn end_to_end_plain_three_segments_via_url_input() {
    ...
    Mock::given(method("GET")).and(path("/v/index.m3u8"))
        .respond_with(ResponseTemplate::new(200).set_body_string(playlist)).mount(&mock).await;
    for i in 0..3 {
        Mock::given(method("GET")).and(path(format!("/v/seg-{i}.ts")))
    ...
    let result = tokio::time::timeout(
        ...
        job.run(DownloadRequest { input: M3u8Input::Url(url), source_url: None, title: "test_plain".into(), headers: HeaderMap::new() }, sink.clone()),
    ).await.expect("no hang").expect("ok");

    let meta = std::fs::metadata(&result.0.0).expect("output exists");
    assert!(meta.len() >= 1024, "tiny output: {}", meta.len());

    let events = sink.snapshot();
    assert!(matches!(events.first(), Some(ProgressEvent::Parsing)), "first event = Parsing");
    let downloads = events.iter().filter(|e| matches!(e, ProgressEvent::Downloading { .. })).count();
    assert_eq!(downloads, 3, "expected 3 download events, got {downloads}");
    assert!(matches!(events.last(), Some(ProgressEvent::Done { .. })), "last event = Done");
}
...
async fn end_to_end_aes128_decrypts_using_cached_key() {
    ...
    Mock::given(method("GET")).and(path("/enc/index.m3u8"))
        .respond_with(ResponseTemplate::new(200).set_body_string(playlist)).mount(&mock).await;
    Mock::given(method("GET")).and(path("/enc/key.bin"))
        .respond_with(ResponseTemplate::new(200).set_body_bytes(key.to_vec())).mount(&mock).await;
    for i in 0..2 {
        let plain = vec![i as u8 + 0x10; 1024];
        let enc = Aes128CbcEnc::new(&key.into(), &iv.into());
        let ct = enc.encrypt_padded_vec_mut::<Pkcs7>(&plain);
        Mock::given(method("GET")).and(path(format!("/enc/seg-{i}.ts")))
    ...
    let result = tokio::time::timeout(
        Duration::from_secs(20),
        job.run(DownloadRequest { input: M3u8Input::Url(url), source_url: None, title: "test_aes".into(), headers: HeaderMap::new() }, sink.clone()),
    ).await.expect("no hang").expect("ok");

    assert!(result.0.0.exists(), "output not written");
    let events = sink.snapshot();
    let dl_count = events.iter().filter(|e| matches!(e, ProgressEvent::Downloading { .. })).count();
...
async fn raw_input_with_source_url_resolves_relative_segments() {
    let mock = MockServer::start().await;
    for i in 0..2 {
        Mock::given(method("GET")).and(path(format!("/raw/seg-{i}.ts")))
    ...
    let result = tokio::time::timeout(
        ...
        job.run(DownloadRequest { input: M3u8Input::Raw(raw.into()), source_url: Some(source), title: "test_raw".into(), headers: HeaderMap::new() }, sink.clone()),
    ).await.expect("no hang").expect("ok");
